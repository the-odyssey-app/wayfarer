...
- Expo CLI
- Git
- SSH access to remote server (configure in `.env.deployment.local`)
...
2. **Edit with your server details**:
   ...
   ```
   Set `DEPLOYMENT_SERVER_HOST`, `DEPLOYMENT_SERVER_USER`, etc.

All deployment scripts automatically use this configuration.
...
1. **Connect to VPS** (using configured host from `.env.deployment.local`):
   ```bash
   # Scripts use centralized config, or manually:
...
3. **Initialize Database** (if first time):
   ```bash
   cd wayfarer-nakama
   # Run complete schema migration (22 tables)
   docker exec -i wayfarer-nakama-cockroachdb-1 cockroach sql --insecure < migrations/001_create_full_schema.sql
   ```
   See **[DATABASE_MIGRATION.md](wayfarer-nakama/DATABASE_MIGRATION.md)** for details.

4. **Access Services** (using configured host):
   - Nakama API: `http://${NAKAMA_HOST}:${NAKAMA_PORT}` (default: 7350)
   - Nakama Console: `http://${NAKAMA_HOST}:${NAKAMA_CONSOLE_PORT}` (default: 7351, admin/password)
   - CockroachDB: `http://${NAKAMA_HOST}:8080`
